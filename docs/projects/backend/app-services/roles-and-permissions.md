...
The best way to write custom filters is by using the API Explorer in conjunction with [Variables](/backend/graphql-api/variables/). The filtered result of any GraphQL query is the scoped record set that can be expected by using the same filter as a role's _Custom Filter_.
