...
All requests made to a workspace endpoint must be run using either a GraphQL client or an HTTP POST request - the only exception is when using [webhook custom functions](/backend/custom-functions/custom-functions
/webhooks). We highly recommend using a GraphQL client like the [8base SDK](https://docs.8base.com/docs/development-tools/sdk/api-client) or [Apollo Client](https://github.com/apollographql/apollo-client).
